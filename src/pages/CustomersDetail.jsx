...
export default function CustomerDetail() {
  ...
  useEffect(() => {
    const loadCustomer = async () => {
      const { data, error } = await supabase
        .from("profiles")
        .select("id, full_name, email, phone, tier, points, role, created_at")
        .eq("id", id)
        .single();
  ...
  if (loading) {
    return (
      <div className="p-4">
        <PageHeader title="Customer Profile" breadcrumb="Dashboard / Customers / Loading" />
        <div className="text-center bg-white p-10 rounded-3xl shadow-sm mt-6">Loading customer data...</div>
      </div>
    );
  }
  ...
  return (
    <div className="p-4">
      <PageHeader title="Customer Profile" breadcrumb={`Dashboard / Customers / ${customer.full_name}`} />
          ...
          <div className="px-8 pb-8">
            <div className="relative flex flex-col md:flex-row items-center md:items-end -mt-16 mb-6 gap-6">
              ...
              <div className="text-center md:text-left flex-1">
                <h2 className="text-3xl font-black text-gray-800">{customer.full_name}</h2>
                <p className="text-gray-500 font-medium">{customer.email}</p>
              </div>

              <div className="mb-2">
                <span className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest shadow-sm ${customer.tier === "Gold" ? "bg-yellow-400 text-white" : customer.tier === "Silver" ? "bg-slate-400 text-white" : "bg-orange-400 text-white"}`}>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                  <p className="text-sm text-gray-600 leading-relaxed">
                    Pelanggan ini merupakan member <span className="font-bold text-green-600">{customer.tier || "Bronze"}</span>.
                    Total poin mereka saat ini adalah <span className="font-bold text-gray-800">{customer.points ?? 0}</span>,
                    yang dapat digunakan untuk meningkatkan tier dan mendapatkan diskon lebih besar.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-10 flex gap-4">
              <Link to="/customers" className="flex-1 text-center py-4 bg-gray-100 text-gray-600 rounded-2xl font-bold hover:bg-gray-200 transition-all">
                Kembali
              </Link>
