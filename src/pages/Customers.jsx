...
const Customers = () => {
  ...
  useEffect(() => {
    const loadCustomers = async () => {
      const { data, error } = await supabase
        .from("profiles")
        .select("id, full_name, email, phone, tier, points")
        .eq("role", "member")
  ...
  return (
    <div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-md bg-white/30 p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white p-8 rounded-3xl w-full max-w-md shadow-2xl border border-white/50 text-left" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-sm font-bold text-gray-700 ml-1">Tier</label>
            ...
            <div className="flex gap-3 mt-8">
              ...
              <button onClick={() => setShowForm(false)} className="flex-1 py-3 bg-green-500 text-white rounded-2xl font-bold shadow-lg shadow-green-200 hover:bg-green-600 active:scale-95 transition-all">
                Save Customer
              </button>
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-x-auto mt-6">
        <table className="w-full text-left">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan="5" className="p-6 text-center text-gray-500">Loading customers...</td>
              </tr>
            ) : customers.length === 0 ? (
              <tr>
                <td colSpan="5" className="p-6 text-center text-gray-500">No customers found.</td>
              </tr>
            ) : (
              customers.map((customer) => (
                <tr key={customer.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                  <td className="p-4 text-sm text-gray-700">{customer.id}</td>
                  ...
                  <td className="p-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${customer.tier === "Gold" ? "bg-yellow-100 text-yellow-600" : customer.tier === "Silver" ? "bg-slate-100 text-slate-600" : "bg-orange-100 text-orange-600"}`}>
